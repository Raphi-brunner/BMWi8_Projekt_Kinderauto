...
void MotorDriver::begin() {
  ...
  ledcSetup(1, 5000, 8);    // Kanal 1, 5 kHz, 8 Bit
...
void MotorDriver::forward(int speed) {
  ...
  stop();
  delay(2);  // Deadtime: 2 ms, kann je nach H-Brücke angepasst werden

  digitalWrite(_AHI, HIGH);
  digitalWrite(_BHI, LOW);

  ledcWrite(0, speed); // PWM Kanal 0 (_ALI)
}
...
void MotorDriver::backward(int speed) {
  ...
  ledcWrite(1, constrain(speed, 0, 170)); // PWM Kanal 1 (_BLI) beschränkt auf 170 (ca. 66% Leistung) für Rückwärtsfahrt.
}
