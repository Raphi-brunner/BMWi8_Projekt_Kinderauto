...
void LED::setModus(uint8_t modus)
{
    ...
    case MODUS_FAHREN:
        ...
        for (int i = HECK_START; i <= HECK_END; i++)
        {
...
void LED::update()
{
    ...
    if (_blinkZustand)
    {
        if (_modus == MODUS_BLINKER_LINKS)
        {
            _bereichFarbe(0, 5, CRGB(255, 120, 0));
            _bereichFarbe(17, 21, CRGB(255, 120, 0));
        }
        else
        {
            _bereichFarbe(5, 16, CRGB(255, 120, 0));
        }
    }
    FastLED.show();
}
